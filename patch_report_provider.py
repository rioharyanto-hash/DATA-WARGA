...
file_path = r"E:\Project\DAWIS\lib\src\features\report\presentation\providers\report_provider.dart"
...
ringkasan_methods = """
  ...
  Future<Uint8List> generateRekapPkkRingkasanPdf() async {
    ...
    try {
      ...
      for (var name in allowedNames) {
        ...
        ringkasanRows.add({
          'namaKrt': name,
          'jmlKk': krtCount, // Number of KRT mapped to JML KK temporarily? Wait, no.
          // Wait, 'namaKrt' expects name, 'jmlKk' expects amount. I'll just use the sums.
  ...
  Future<Uint8List> generateProfilPendudukRingkasanPdf() async {
    ...
    try {
      ...
      // In Profil Penduduk, each row has age groups 0-1_P, 0-1_W, etc.
      // We will sum these up per kelompok.
      for (var name in allowedNames) {
        ...
        sums['namaKeluarga'] = 0; // Won't be used, we put NAMA in name instead
  ...
  Future<Uint8List> generatePotensiWargaRingkasanPdf() async {
    ...
    try {
      ...
      // In Potensi Warga, each row has values like j_L, j_P, dll.
      for (var name in allowedNames) {
...
index = content.rfind("}")
if index != -1:
    new_content = content[:index] + ringkasan_methods + content[index:]
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Patched report_provider.dart")
else:
