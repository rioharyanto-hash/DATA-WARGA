...
void main() {
  ...
  // We want to replace the part that builds dataRows and returns it with a pw.Table
  // From: `List<pw.Widget> dataRows = [];`
  // To: `return dataRows;`

  String oldStrStart = 'List<pw.Widget> dataRows = [];';
  String oldStrEnd = 'return dataRows;';
  
  int startIdx = content.indexOf(oldStrStart);
  int endIdx = content.indexOf(oldStrEnd, startIdx);
  ...
  String after = content.substring(endIdx + oldStrEnd.length);
  ...
  String newLogic = r'''
          ...
          return [
            pw.Table(
              ...
              columnWidths: {
                for (int j = 0; j < 30; j++)
                  j: pw.FlexColumnWidth(j == 1 ? 4 : 1),
              },
              children: tableRows,
            )
          ];''';

  file.writeAsStringSync(before + newLogic + after);
  print('Success');
}
